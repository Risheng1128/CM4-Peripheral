...
# ChangeLog :
#	2021-8-31 - first version
#   2022-1-26 - second version
# ------------------------------------------------
...
CPU = -mcpu=cortex-m4 # -mcpu=name 指定ARM處理器的型號，如“cortex-m4”等
FPU = -mfpu=fpv4-sp-d16 # -mfpu=name 指定硬體FPU型號，如“vfpv2”等
FLOAT-ABI = -mfloat-abi=hard # -mfloat-abi=name 指定要使用的浮點ABI，可以是 "hard" 或 "soft"
...
# -lc -lm -lnosys c 庫、數學庫、nosys庫，用於驗證連結是否有效。
LIBS = -lc -lm -lnosys
